...
def make_pattern(name, exp_list):
    ...
    pattern = MeteorPatternScaling(
        name,
        {"tas": 2, "pr": 10},
        lambda exp: os.path.join(
            "/div/no-backup/users/masan/temp", f"pdrmip-{exp}_T42_ANN_NorESM1.nc"
        ),
        exp_list=exp_list,
    )
    return pattern
...
def predict_per_experiment(pattern, experiment):
    conc_data = input_handler.read_inputfile(
        os.path.join(
            "/div/amoc/CSCM/SCM_Linux_v2019/RCMIP/input", f"{experiment}_conc_RCMIP.txt"
        )
    )
    ih = input_handler.InputHandler({})
    em_data = ih.read_emissions(
        os.path.join("../tests/test-data", f"{experiment}_em_RCMIP.txt")
    )
    patterns = pattern.predict_from_combined_experiment(
...
def add_rcp_data(ax, comp, exp, top_figure_ax):
    data_hist = xr.open_dataset(
        f"/div/no-backup/users/masan/temp/{comp}_yr_NorESM1-M_historical_r1i1p1_185001-200512.nc"
    )[comp]
    data_rcp = xr.open_dataset(
        f"/div/no-backup/users/masan/temp/{comp}_yr_NorESM1-M_{exp}_r1i1p1_200601-210012.nc"
    )[comp]
    hist_ts = prpatt.global_mean(data_hist).values
    tot_ts = np.concatenate(
        (hist_ts - hist_ts[0], prpatt.global_mean(data_rcp).values - hist_ts[0])
    )
    top_figure_ax.plot(np.arange(1850, 2101), tot_ts, label="CMIP5")
    mean_change = data_rcp[-20:, :, :].mean(dim="time") - data_hist[:50, :, :].mean(
        dim="time"
    )
    if comp == "tas":
...
def add_pattern_data(patterns, ax, comp, top_figure_ax, label):
    top_figure_ax.plot(
        np.arange(1750, 2101),
        prpatt.global_mean(patterns[comp]),
        linestyle="--",
        alpha=0.8,
        label=label,
    )
    mean_change = patterns[comp][-20:, :, :].mean(dim="time") - patterns[comp][
        100:150, :, :
    ].mean(dim="time")
    mean_change.plot(ax=ax, cmap="bwr")
    ax.set_xlabel(None)
    ...
    for column in range(len(experiments)):
        ...
        for col, ax in enumerate(axs):
            ...
            if row == 0:
                ...
                add_pattern_data(
                    patterns, ax, comp, top_figures[column * 2 + col], labels[row - 1]
                )
            ax.set_title(comp)
