...
checks: $(VENV_DIR)  ## run all the checks
	@echo "=== bandit ==="; $(VENV_DIR)/bin/bandit -c .bandit.yml -r src || echo "--- bandit failed ---" >&2; \
		...
		echo "\n\n=== notebook tests 2 ==="; $(VENV_DIR)/bin/pytest notebooks/CMIP6_mmdemo_with_archiving.ipynb -r a --nbval --nbval-sanitize-with $(NOTEBOOKS_SANITIZE_FILE) || echo "--- notebook tests failed ---" >&2; \
