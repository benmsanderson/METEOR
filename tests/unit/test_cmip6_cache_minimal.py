...
class TestCacheCore:
    ...
    def test_cache_key_generation(self, cache_getter):
        ...
        keys = {
            "raw": cache_getter._generate_cache_key("get_single_var_mod_data", "piControl", "tas", "CanESM5"),
            "monthly": cache_getter._generate_cache_key("get_single_var_mod_data_monthly", "piControl", "tas", "CanESM5"),
            "yearly": cache_getter._generate_cache_key("get_single_var_mod_data_yearmean", "piControl", "tas", "CanESM5"),
        }
    ...
    def test_cache_lifecycle(self, cache_getter, temp_cache_dir):
        ...
        # 2. Validate
        is_valid = cache_getter.is_cached("get_single_var_mod_data_monthly", "piControl", "tas", "TestModel")
    ...
    def test_validation_rejects_corruption(self, cache_getter):
        ...
        scenarios = [
            ...
            ("wrong_var", xr.Dataset({"pr": xr.DataArray([1, 2, 3], dims=["time"])}), False),
            ("no_dims", xr.Dataset({"tas": xr.DataArray(42.0)}), False),
            ("valid", xr.Dataset({"tas": xr.DataArray([1, 2, 3], dims=["time"], coords={"time": [0, 1, 2]})}), True),
        ]
    ...
    def test_compression_effectiveness(self, cache_getter, temp_cache_dir):
        ...
        assert compressed_size < 100_000, "Compression should significantly reduce file size"
...
class TestCacheErrorHandling:
    ...
    def test_wrong_variable_rejection(self, cache_getter, temp_cache_dir):
        ...
        wrong_ds = xr.Dataset({
            "pr": xr.DataArray(np.random.rand(10, 5, 5), dims=["time", "lat", "lon"], coords={"time": range(10)})
        })
        wrong_ds.attrs["cached_by_meteor"] = "true"
        
        cache_key = cache_getter._generate_cache_key("get_single_var_mod_data_monthly", "piControl", "tas", "TestModel")
        cache_path = cache_getter._get_cache_path(cache_key)
        wrong_ds.to_netcdf(cache_path)
        
        # is_cached should reject and clean up
        is_cached = cache_getter.is_cached("get_single_var_mod_data_monthly", "piControl", "tas", "TestModel")
        assert not is_cached
        assert not os.path.exists(cache_path)

    def test_cache_disabled_no_operations(self):
        """Test that disabled cache skips all operations."""
        getter = Cmip6MeteorDataGetter(flds=["tas"], exps=["piControl"], enable_cache=False)
        
        # All cache operations should return False/None immediately
        assert not getter.is_cached("get_single_var_mod_data_monthly", "piControl", "tas", "Model")
...
class TestCacheFeatures:
    ...
    def test_time_bnds_removal(self, cache_getter):
        ...
        test_ds = xr.Dataset({
            "tas": xr.DataArray(np.random.rand(10, 5, 5), dims=["time", "lat", "lon"], coords={"time": range(10)}),
