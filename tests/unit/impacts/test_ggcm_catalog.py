...
class TestCatalogContent:
    """Spot-checks that the catalog reflects the paper's Table 1."""

    def test_all_nine_models_present(self):
...
class TestIsAvailable:
    ...
    def test_returns_false_for_unknown_crop(self):
        assert not catalog.is_available("LPJmL", "quinoa", "A0")

    def test_returns_false_for_unknown_variant(self):
        assert not catalog.is_available("LPJmL", "maize", "A2")

    def test_returns_true_for_known_combination(self):
...
class TestGetDownloadUrl:
    def test_url_contains_record_id(self):
    ...
    def test_url_raises_for_unavailable(self):
        with pytest.raises(ValueError, match="Not available"):
            catalog.get_download_url("JULES", "maize", "A1")

    def test_url_raises_for_unknown_model(self):
...
class TestGetAvailable:
    ...
    def test_get_available_variants_known(self):
        variants = catalog.get_available_variants("LPJmL", "maize")
        assert set(variants) == {"A0", "A1"}

    def test_get_available_variants_A0_only(self):
        variants = catalog.get_available_variants("JULES", "maize")
        assert variants == ["A0"]

    def test_get_available_variants_unknown_model(self):
        assert catalog.get_available_variants("UNKNOWN", "maize") == []

    def test_get_available_variants_unknown_crop(self):
...
class TestGgcmDownloader:
    def test_get_filepath_returns_correct_path(self, tmp_path):
    ...
    def test_ensure_files_raises_for_unavailable_crop(self, tmp_path):
        dl = GgcmDownloader(str(tmp_path))
        with pytest.raises(ValueError, match="not available"):
            dl.ensure_files(["quinoa"], "LPJmL", "A0")

    def test_ensure_files_raises_for_unavailable_model(self, tmp_path):
        dl = GgcmDownloader(str(tmp_path))
        with pytest.raises(ValueError):
            dl.ensure_files(["maize"], "FAKE_MODEL", "A0")

    def test_ensure_files_raises_for_JULES_A1(self, tmp_path):
        dl = GgcmDownloader(str(tmp_path))
        with pytest.raises(ValueError):
            dl.ensure_files(["maize"], "JULES", "A1")

    def test_ensure_files_returns_early_if_all_present(self, tmp_path):
    ...
    def test_ensure_files_raises_import_error_when_requests_missing(self, tmp_path):
    ...
    def test_ensure_files_downloads_missing_files(self, tmp_path):
    ...
    def test_download_file_streams_chunks(self, tmp_path):
        ...
        dl = GgcmDownloader(str(tmp_path))
        dl._download_file(
            "http://example.com/test.nc4", dest, mock_requests, mock_tqdm_cls
        )
    ...
    def test_download_file_resumes_partial(self, tmp_path):
