...
class TestAnomályConversion:
    ...
    def test_tas_converted_to_anomalies(self, mock_interface):
        ...
        picontrol_data = xr.Dataset(
            {
                "tas": xr.DataArray(
                    np.full((100, 5, 5), picontrol_mean)
                    + np.random.randn(100, 5, 5) * 0.1,
                    dims=["month", "lat", "lon"],
                    coords={
                        "month": range(100),
    ...
    def test_pr_not_converted_to_anomalies(self, mock_interface):
        ...
        mock_interface.noise_models[
            "pr"
        ].generate_regional_mean_realizations.return_value = (
            np.random.rand(3, 100) * 1e-5
        )
    ...
    def test_anomaly_subtraction_correct(self):
        ...
        picontrol_data = xr.Dataset(
            {
                "tas": xr.DataArray(
                    np.full((10, 3, 3), picontrol_mean), dims=["month", "lat", "lon"]
                )
            }
        )
        ...
        scenario_data = xr.Dataset(
            {
                "tas": xr.DataArray(
                    np.full((10, 3, 3), scenario_value), dims=["month", "lat", "lon"]
                )
            }
        )
...
class TestTrainingWorkflow:
    ...
    def test_train_initializes_state_tracking(self):
        ...
        with patch(
            "meteor.meteor_interface.Cmip6MeteorDataGetter"
        ) as mock_getter_class:
