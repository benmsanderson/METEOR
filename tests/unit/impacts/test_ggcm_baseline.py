...
class TestAgmerraShape:
    def test_temperature_shape(self, agmerra):
        T_agmerra, _ = agmerra
    ...
    def test_precipitation_shape(self, agmerra):
        _, W_agmerra = agmerra
        assert W_agmerra.shape == (360, 720)


class TestAgmerraTypes:
    def test_temperature_dtype(self, agmerra):
        T_agmerra, _ = agmerra
        assert T_agmerra.dtype == np.float64

    def test_precipitation_dtype(self, agmerra):
        _, W_agmerra = agmerra
        assert W_agmerra.dtype == np.float64

    def test_temperature_is_ndarray(self, agmerra):
        T_agmerra, _ = agmerra
    ...
    def test_precipitation_is_ndarray(self, agmerra):
        _, W_agmerra = agmerra
        assert isinstance(W_agmerra, np.ndarray)
        assert not isinstance(W_agmerra, np.ma.MaskedArray)


class TestAgmerraPhysicalPlausibility:
    def test_temperature_range(self, agmerra):
    ...
    def test_precipitation_floor(self, agmerra):
    ...
    def test_precipitation_no_negative(self, agmerra):
        _, W_agmerra = agmerra
        assert np.all(W_agmerra >= 0.0)

    def test_temperature_has_spatial_variation(self, agmerra):
    ...
    def test_precipitation_has_spatial_variation(self, agmerra):
        _, W_agmerra = agmerra
