...
def test_returns_array_with_correct_shape():
    """K shape must be (35, 360, 720) as stored in the Zenodo files."""
    k_data = np.zeros((35, 360, 720), dtype=np.float64)
    mock_nc = _make_mock_nc(k_data)
    with patch("meteor.impacts.ggcm.coefficients.netcdf.Dataset", return_value=mock_nc):
        K = load_coefficients("fake.nc4")
    assert K.shape == (35, 360, 720)


def test_returns_float64_dtype():
...
def test_dataset_opened_in_read_mode():
...
def test_dataset_is_closed_after_read():
