...
def apply_distribution_transform(gaussian_data, gaussian_params, target_params, 
                                 target_dist='gamma'):
    """
    ...
    if is_scalar_params:
        ...
        if target_dist == 'gamma':
            transformed = stats.gamma.ppf(uniform, a=target_params['shape'], 
                                         scale=target_params['scale'])
        elif target_dist == 'weibull':
            transformed = stats.weibull_min.ppf(uniform, c=target_params['shape'], 
                                               scale=target_params['scale'])
        elif target_dist == 'lognorm':
            transformed = stats.lognorm.ppf(uniform, s=target_params['shape'], 
                                           scale=target_params['scale'])
        elif target_dist == 'gengamma':
            transformed = stats.gengamma.ppf(uniform, a=target_params['a'], 
                                            c=target_params['c'], 
                                            scale=target_params['scale'])
        else:
        ...
        uniform = stats.norm.cdf(data_reshaped, loc=mean_grid[None, :], scale=std_grid[None, :])
        uniform = np.clip(uniform, 1e-10, 1 - 1e-10)
        ...
        if target_dist == 'gamma':
            ...
            transformed_flat = stats.gamma.ppf(uniform, a=shape_grid[None, :], 
                                              scale=scale_grid[None, :])
        else:
            raise ValueError(f"Only 'gamma' distribution supported for 3D data, got {target_dist}")
