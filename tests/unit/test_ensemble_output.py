...
from meteor.ensemble_output import VariableOutput, EnsembleOutput
...
class TestVariableOutput:
    ...
    def test_list_timeseries_empty(self):
        """Test list_timeseries when empty."""
        var_out = VariableOutput('tas')
        assert var_out.list_timeseries() == []
    
    def test_list_timeseries_populated(self):
    ...
    def test_list_gridded_empty(self):
        """Test list_gridded when empty."""
        var_out = VariableOutput('tas')
        assert var_out.list_gridded() == []
    
    def test_list_gridded_populated(self):
    ...
    def test_list_impacts_empty(self):
        """Test list_impacts when empty."""
        var_out = VariableOutput('tas')
        assert var_out.list_impacts() == []
    
    def test_list_impacts_populated(self):
...
class TestEnsembleOutput:
    ...
    def test_list_variables_populated(self):
    ...
    def test_to_netcdf_with_special_characters(self):
        ...
        try:
            ...
            assert 'tas_point_59p9,10p8' in loaded.data_vars or 'tas_point_59p9_10p8' in loaded.data_vars
            loaded.close()
        finally:
            if os.path.exists(tmp_path):
                os.remove(tmp_path)
    
    def test_to_netcdf_with_impacts(self):
        """Test saving with impact metrics included."""
        var_tas = VariableOutput('tas')
        var_tas.timeseries['global'] = xr.DataArray([290.0], dims=['time'])
        var_tas.impacts['hdd'] = {
            'global': xr.DataArray([1500.0], dims=['time'])
        }
    ...
    def test_to_netcdf_without_impacts(self):
    ...
    def test_to_netcdf_with_metadata_attributes(self):
    ...
    def test_to_netcdf_multiple_variables(self):
