...
data_getter = Cmip6MeteorDataGetter(
    exps=["piControl", "abrupt-4xCO2"], dbe=["CMIP", "CMIP"]
)
...
for i, model in enumerate(models):
    ...
    for j, fld in enumerate(flds):
        truth_abrupt = prpatt.global_mean(
            data_getter.get_single_var_mod_data_yearmean("abrupt-4xCO2", fld, model)
        ).values[0]
        truth_picontrol = prpatt.global_mean(
        ...
        data_in_pattern = prpatt.global_mean(
            model_basic_pattern.dacanom[fld][
                model_basic_pattern.exp_list.index("co2x4"), :100, :, :
            ]
        )
        # print(f"Variable: {fld} and model {model}: {hist_time}")
        axs[i * len(flds) + j, 0].plot(
            years_total,
            truth_abrupt - truth_picontrol[: len(truth_abrupt)],
            color="red",
            label="CMIP6_abrupt",
        )
        axs[i * len(flds) + j, 2].plot(
            years_pic,
            truth_picontrol - truth_picontrol[0],
            color="red",
            label="CMIP6_piControl",
        )
        meteor_plot_utils.plot_global_mean_values(
            model_basic_pattern, axs[i * len(flds) + j, 0], fld, "co2x4"
        )
        axs[i * len(flds) + j, 1].plot(range(100), truth_abrupt[:100] - data_in_pattern)
        meteor_plot_utils.plot_global_mean_values(
            model_basic_pattern, axs[i * len(flds) + j, 2], fld, "base"
        )
