...
def fit_timescales(X, a0):
    ...
    print(f"  Computing global mean timeseries...")
    ...
    if n_valid < len(ts):
        print(f"  ⚠️  WARNING: Timeseries contains NaN values!")
        ...
        if len(valid_indices) > 0:
            ...
            if last_valid_idx < len(ts) - 1:
                print(f"  ⚠️  NaNs detected at end of timeseries (after month {last_valid_idx})")
                ts = ts[:last_valid_idx + 1]
            else:
                # NaNs are scattered - use only contiguous valid data from start
                first_nan_idx = np.where(~valid_mask)[0][0] if np.any(~valid_mask) else len(ts)
    ...
    # print(ts)
    print(f"  Starting lmfit.minimize...")
    try:
        out = lmfit.minimize(
        ...
        if not out.success:
            print(f"  ⚠️  Optimization failed!")
            print(f"  Final parameters: {out.params.valuesdict()}")
        return out
    except Exception as fit_error:
        print(f"\n{'='*70}")
        print(f"❌ LMFIT MINIMIZE FAILED!")
        print(f"{'='*70}")
        ...
        print(f"  Full timeseries values:")
        print(ts)
        print(f"\n  Statistics:")
...
def get_timescales(anomaly_data, n_modes):
    ...
    print(f"  anomaly_data range: [{np.nanmin(anomaly_data.values):.6e}, {np.nanmax(anomaly_data.values):.6e}]")
