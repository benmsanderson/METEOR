...
from meteor.meteor_interface import (
    MeteorInterface,
    _get_default_config,
    _load_emissions_concentrations,
    _parse_scenario_input,
)
...
def test_parse_scenario_input():
    ...
    scenario_str = "ssp245"
    result = _parse_scenario_input(scenario_str)
    ...
    result = _parse_scenario_input(scenario_dict)
    ...
    result = _parse_scenario_input(scenario_dict_no_conc)
    ...
    with pytest.raises(
        ...
        _parse_scenario_input({"Hello": "invalid"})  # Invalid type

    with pytest.raises(TypeError, match="scenario must be str or dict"):
        _parse_scenario_input(12345)  # Invalid type
...
def test_load_emissions_concentrations(test_data_dir):
    ...
    with pytest.raises(TypeError, match="emissions must be str path or DataFrame"):
        _load_emissions_concentrations(123, 123)
    with pytest.raises(TypeError, match="concentrations must be str path or DataFrame"):
        _load_emissions_concentrations(df_min, 123)
    em_data, conc_data = _load_emissions_concentrations(df_min, df_min, verbose=True)
    pd.testing.assert_frame_equal(em_data, df_min)
    pd.testing.assert_frame_equal(conc_data, df_min)
    em_data, conc_data = _load_emissions_concentrations(df_min, df_min)
    pd.testing.assert_frame_equal(em_data, df_min)
    pd.testing.assert_frame_equal(conc_data, df_min)
    ...
    em_data2, conc_data2 = _load_emissions_concentrations(
        os.path.join(test_data_dir, "rcp85_em_RCMIP.txt"),
        os.path.join(test_data_dir, "rcp85_conc_RCMIP.txt"),
        verbose=True,
    )
